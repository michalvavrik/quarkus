...
HTTP/3 (QUIC) is available as an experimental extension.
It uses QUIC over UDP instead of TCP and requires TLS.

To enable HTTP/3, add the `quarkus-http3` extension to your project.
See the xref:http3-reference.adoc[HTTP/3 reference guide] for configuration and usage details.
...
Quarkus allows users to arbitrarily customize the configuration of HTTP servers started by Quarkus via the use of `io.quarkus.vertx.http.HttpServerConfigCustomizer`.
...
@Singleton <1>
public class MyCustomizer implements HttpServerConfigCustomizer {

    @Override
    public void customizeHttpServer(HttpServerConfig config) { <2>
        config.setPort(9998);
    }
}
----
<1> By making the class a managed bean, Quarkus will take the customizer into account when it starts the Vert.x servers
<2> In this case, we only care about customizing the HTTP server, so we just override the `customizeHttpServer` method, but users should be aware that `HttpServerConfigCustomizer` allows configuring the HTTPS and Domain Socket servers as well
...
The following example uses the xref:./tls-registry-reference.adoc[TLS centralized configuration] to set up mTLS and proxy trust:
...
By default, the `proto` values from `Forwarded` and `X-Forwarded-Proto` headers are validated against the https://datatracker.ietf.org/doc/html/rfc3986#section-3.1[URI scheme syntax].
...
When the header holds fewer values than the index, the request is rejected with a `400 Bad Request` response.
...
----
<1> Falls back to the direct connection address. Returning `null` instead rejects the request with a `400 Bad Request` response.
