...
The proxy is trusted when its certificate's Subject DN contains all components specified in the configured value.
...
quarkus.http.proxy.trusted-proxy[0].truststore-alias=my-proxy-ca <1>
----
<1> The alias is resolved from the HTTP server's TLS registry truststore. Legacy `quarkus.http.ssl` certificate properties are not supported for alias resolution.

Alternatively, a truststore alias can be used without a Subject DN.
Any client whose certificate chain matches the trust anchor under the given alias is then trusted as a proxy.
Ensure the alias identifies only the proxy — for example, by importing the proxy's leaf certificate rather than a CA that also signs non-proxy clients:

[source,properties]
----
quarkus.http.proxy.trusted-proxy[0].truststore-alias=proxy-cert
----
