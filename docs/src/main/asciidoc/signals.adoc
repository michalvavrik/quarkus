...
If the xref:security-overview.adoc[Quarkus Security extension] is present, the `SecurityIdentity` of the emitting thread is automatically propagated to each receiver invocation.

The identity is captured at emission time and restored in the receiver's request context.
Receiver methods can therefore inject `SecurityIdentity` and use standard security annotations such as `@RolesAllowed`, `@Authenticated`, `@PermitAll`, `@DenyAll`, and `@PermissionsAllowed`.

[source,java]
----
package org.acme.signals.security;
...
public class OrderHandlers {
    ...
    @RolesAllowed("admin")
    void onOrder(@Receives OrderPlaced order) { // <1>
        Log.infof("Order %s processed by %s", order.orderId(), identity.getPrincipal().getName());
    }
}
----
<1> Only invoked if the `SecurityIdentity` propagated from the emitter has the `admin` role. An `UnauthorizedException` or `ForbiddenException` is thrown otherwise.

=== When is the identity available?

The identity is only propagated when it has already been resolved at the moment the signal is emitted.
This happens automatically when xref:security-proactive-authentication.adoc[proactive authentication] is enabled (the default).

If proactive authentication is disabled, the identity is still propagated provided it was resolved before the emission -- for example, because an authorization check (like `@RolesAllowed`) was performed or the `SecurityIdentity` was accessed earlier in the request:
...
public class OrdersResource {
    ...
    record OrderPlaced(String orderId, String username) {
    }
    ...
    @RolesAllowed("user") // <1>
    @GET
    public String placeOrderIfUser() {
        orderPlaced.publish(new OrderPlaced("ORD-1", null));
        return "ok";
    }

    @GET
    public String placeOrderWithUsername() {
        String username = securityIdentity.getPrincipal().getName(); // <2>
        orderPlaced.publish(new OrderPlaced("ORD-1", username));
        return "ok";
    }
}
----
<1> The `@RolesAllowed` check forces authentication, so the identity is resolved before the signal is emitted.
<2> Accessing the `SecurityIdentity` principal triggers authentication, so the identity is available for propagation.

NOTE: If no identity has been resolved at emission time (for example, an unauthenticated request with proactive authentication disabled and no authorization check), receivers see an anonymous identity. When the propagated identity carries an expiration time (e.g. from an OIDC token), it is automatically discarded once expired -- receivers will then see an anonymous identity and security checks like `@Authenticated` will deny access.
