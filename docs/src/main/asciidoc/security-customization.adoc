...
Quarkus beans fire various security events that can be consumed by xref:cdi.adoc#events-and-observers[CDI observers].
All security events implement the `io.quarkus.security.spi.runtime.SecurityEvent` interface, which allows you to consume all the events with one observer method.
...
public class SecurityEventObserver {
    ...
    void observe(@ObservesAsync SecurityEvent event) {              <1>
        SecurityIdentity identity = event.getSecurityIdentity();
    }

    void observe(@Observes AuthenticationFailureEvent event) {
        RoutingContext routingContext = (RoutingContext) event      <2>
                .getEventProperties()
                .get(RoutingContext.class.getName());
    }

    void observe(@Observes AuthorizationFailureEvent event) {       <3>
        Throwable failure = event.getAuthorizationFailure();
    }
}
----
<1> This observer consumes all the security events asynchronously, which means that HTTP request processing will continue regardless on the event processing.
The `SecurityEvent` observer method is notified with every authentication and authorization success, depending on the application, that can be a lot of events.
For that reason, asynchronous processing can have positive effect on performance.
<2> Applications based on the Vert.x HTTP extension should be able to access the `RoutingContext` from event properties.
<3> The `AuthorizationFailureEvent` event is fired when request does not comply a standard security annotation constrain or is denied access by an HTTP Security policy.
