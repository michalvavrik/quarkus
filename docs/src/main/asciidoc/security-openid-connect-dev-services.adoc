...
=== Bearer token authentication testing with `OidcTestClient`

Quarkus provides easy way to retrieve access token from the started OIDC server:
...
----
<1> The `OidcTestClient` can be used in the `@QuarkusTest` or `@QuarkusIntegrationTest`.

For information how to use the `OidcTestClient` for testing, please refer to the xref:security-oidc-bearer-token-authentication-tutorial.adoc#integration-testing-oidc-test-client[OidcTestClient] section of the OIDC Bearer token authentication guide.
