...
<2> The HTTP permission `roles1` secures the `/*` path. Only authenticated HTTP requests are authorized to access application endpoints.

Example below shows how to map the `SecurityIdentity` roles to the deployment-specific roles for all the paths without securing the `/*` path:

[source,properties]
----
quarkus.security.roles.admin=Admin1 <1>
----
<1> The `SecurityIdentity` will have mapped same roles like in the example before.
However, the `/*` path is not secured by the role-based policy.
You can either annotate your endpoints with standard security annotations or define additional path-specific HTTP permissions.
