...
public class CRUDResource {
    ...
    @PermissionsAllowed("resource\\:name:action") <4>
    @GET
    @Path("/resource")
    ...
    @PermissionsAllowed(value = "list", permission = CustomPermission.class) <5>
    @Path("/list")
    @GET
    ...
    public static class CustomPermission extends BasicPermission {
        ...
        public boolean implies(Permission permission) {
            var event = Arc.container().instance(RoutingContext.class).get(); <6>
...
<4> The colon separates the permission name from the action.
To include a literal colon in the permission name, prefix it with a backslash (`\\:`).
Here, the permission name is `resource:name` and the action is `action`.
The backslash character is only allowed immediately before a colon.
A value with more than one unescaped colon is rejected because there is no way to determine which colon is the separator.
A single unescaped colon at the very start or end of the value is treated as part of the name, not as a separator.
<5> You can use your preferred `java.security.Permission` implementation.
By default, string-based permission is performed by `io.quarkus.security.StringPermission`.
<6> Permissions are not beans, therefore the only way to obtain bean instances is programmatically by using `Arc.container()`.
...
quarkus.http.auth.policy.role-policy3.permissions.user=resource\\\\:name:action                      <5>
quarkus.http.auth.permission.roles3.paths=/crud/resource
quarkus.http.auth.permission.roles3.policy=role-policy3
----
...
When escaped colons are used, the `@PermissionChecker` value must be an exact match of the raw `@PermissionsAllowed` value, including the escape characters:

[source,java]
----
@PermissionsAllowed("ns\\:resource:read") <1>
void matched() { }

@PermissionsAllowed("ns:resource:read") <2>
void notMatched() { }

@PermissionChecker("ns\\:resource:read")
boolean canAccess(SecurityIdentity identity) {
    return identity.hasRole("admin");
}
----
<1> Matched by the permission checker because both annotation values are `ns\:resource:read`.
<2> Not matched because the raw value `ns:resource:read` differs from the checker value `ns\:resource:read`.
