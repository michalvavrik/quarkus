...
On the other hand, the `*` wildcard used inside path represents exactly one path segment.
...
The `*` wildcard used inside path, not at the end, can be used as replacement for exactly one path segment, consisting of any HTTP path characters.
Let's consider an HTTP request to the `/one/two/three/four/five` path with 5 path segments (`one`, `two`, `three`, `four`, `five`).
Valid pattern using the wildcard to represent one path segment inside path, like `/one/two/*/four/five`, will match as
long as it is the most specific path pattern. The longest segment sequence without wildcard is considered the most specific.
On the other hand, the path `/one/two/*/five` will never be matched, for the wildcard can only represent more than a one segment when used at then end.
Here are some of possible path matches ordered from the most specific to the least specific:
...
NOTE: The `*` wildcard inside path can be only used as standalone segment character, representing all the segment characters. Thus, paths like `/one/tw*/three` are invalid.
...
quarkus.http.auth.policy.root-policy1.roles-allowed=root

quarkus.http.auth.permission.roles1.paths=/api/*,/restricted/* <1>
quarkus.http.auth.permission.roles1.policy=user-policy1

quarkus.http.auth.permission.roles2.paths=/api/*,/admin/*
quarkus.http.auth.permission.roles2.policy=admin-policy1

quarkus.http.auth.permission.roles3.paths=/foo/*/bar
quarkus.http.auth.permission.roles3.policy=user-policy1

quarkus.http.auth.permission.roles4.paths=/foo/*/bar
quarkus.http.auth.permission.roles4.policy=admin-policy1

quarkus.http.auth.permission.roles5.paths=/foo/baz/bar <2>
quarkus.http.auth.permission.roles5.policy=root-policy1
----
<1> Given the preceding permission set, `GET /api/foo` would match both permission sets' paths, requiring both the `user` and `admin` roles.
<2> When the `*` wildcard is used anywhere else but at the end, only exact pattern matches can win simultaneously. Thus the path `/foo/baz/bar` requires the `root` role, while `/foo/fum/bar` requires both `user` and `admin` roles.
