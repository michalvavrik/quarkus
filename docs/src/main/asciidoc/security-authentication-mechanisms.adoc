...
=== Order built-in authentication mechanisms

The Quarkus Security provides default priority for built-in security authentication mechanisms such as Basic, Form-based, and mutual TLS (mTLS) authentication mechanisms.
This priority determines order, in which the mechanisms are asked to challenge the authentication request when the incoming request provided wrong credentials.
...
^|3000 for the inclusive authenticate, otherwise 1000
