...
|Email address |xref:security-email-authentication-reference.adoc[Email authentication]
...
Authenticates users based on their access to their email account.
Users receive an email authentication token via email.
This mechanism is suitable for small applications without access to OpenID Connect providers.
For more information, see the xref:security-email-authentication-reference.adoc[Email authentication] guide.
...
<7> The REST endpoint `/hello/basic-or-code-flow` can be accessed either by using the xref:security-oidc-code-flow-authentication.adoc[OIDC authorization code flow mechanism], or by using the <<basic-auth>>.
...
==== Use annotations to enable inclusive path-based authentication for Jakarta REST endpoints
...
----
<1> The REST endpoint `/hello/basic-and-code-flow` can be accessed only if both the xref:security-oidc-code-flow-authentication.adoc[OIDC authorization code flow mechanism] and the <<basic-auth>> have successfully provided `SecurityIdentity`.
...
----
<1> The REST endpoint `/hello/basic-and-code-flow` uses both the xref:security-oidc-code-flow-authentication.adoc[OIDC authorization code flow mechanism] and the <<basic-auth>>.
Access is granted if at least one of them successfully provides `SecurityIdentity`.
