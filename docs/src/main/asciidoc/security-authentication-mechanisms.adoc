...
=== Configuring authentication mechanism priority

Quarkus Security uses a priority system to determine which mechanism gets the first chance to produce that `SecurityIdentity`.
This priority affects the order of:

* **Credential Verification:** Mechanisms with higher priority are checked first. The first one to validate credentials wins, stopping the process (unless inclusive authentication is enabled).
* **Authentication Challenge:** If the request is unauthenticated, mechanisms are asked to issue a challenge in priority order.

The default priorities can be overridden using the configuration properties listed below.

.Supported authentication mechanism priority
|===
...
^|3000 (if inclusive authentication is enabled), otherwise 1000
