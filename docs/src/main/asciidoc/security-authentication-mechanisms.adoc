...
=== Priority of authentication mechanisms

Quarkus Security uses a priority system to determine which mechanism gets the first chance to produce `SecurityIdentity`.
The authentication mechanism priority affects the order of:

* **Credential Verification:** Mechanisms with a higher priority are asked to verify credentials first. The first one to validate credentials wins.
* **Authentication Challenge:** If the request is unauthenticated, mechanisms with a higher priority are asked to issue a challenge first.

The default priorities can be overridden using the configuration properties listed below.

.Priorities of the supported authentication mechanisms
|===
^|Authentication mechanism^| Configuration property^| Default priority

s|Basic authentication mechanism
^|`quarkus.http.auth.basic.priority`
^|2000

s|Form-based authentication mechanism
^|`quarkus.http.auth.form.priority`
^|1000

s|Mutual TLS authentication mechanism
^|`quarkus.http.auth.mtls.priority`
^|1000 by default, 3000 when an inclusive authentication is enabled

ifndef::no-webauthn-authentication[]
s|WebAuthn authentication mechanism
^|`quarkus.webauthn.priority`
^|1000
endif::no-webauthn-authentication[]

s|OIDC authentication mechanism
^|`quarkus.oidc.priority`
^|1001

s|SmallRye JWT authentication mechanism
^|`quarkus.smallrye-jwt.priority`
^|1000
|===
...
[NOTE]
====
When an inclusive authentication is enabled, the <<mutual-tls>> mechanism has the highest priority by default, to reflect on the fact that when it is used, it is enforced early at the transport level, and to ensure that an injected `SecurityIdentity` represents a <<mutual-tls>> authentication.
If you need an injected `SecurityIdentity` to represent another authentication mechanism, <<authentication-mechanisms-priority, configure the mechanism priority>>.
====
...
==== Authorization with multiple identities

Authorization is always performed on the first produced `SecurityIdentity`. For example:
...
----
<1> The `deleteEntity` method can only be accessed if the first `SecurityIdentity` has the `admin` role. <<authentication-mechanisms-priority, Configure the mechanism priority>> to impact which authentication mechanism is used to produce the first `SecurityIdentity`.

If you want to perform authorization on all `SecurityIdentity` instances, you must do it manually.
For example, to check if a permission is allowed at least by one of the identities, use a `@PermissionChecker` annotation:
...
[IMPORTANT]
====
Ensure that the value of the `auth-mechanism` property matches the authentication scheme supported by `HttpAuthenticationMechanism`, for example, `basic`, `bearer` or `custom`.
...
When <<inclusive-authentication>> is enabled for all the HTTP paths with the `quarkus.http.auth.inclusive` configuration property, every selected authentication mechanism must provide the `SecurityIdentity`.
For example:
...
----
<1> The REST endpoint `/hello/basic-bearer` can be accessed by using both the xref:security-oidc-bearer-token-authentication.adoc[OIDC bearer token authentication mechanism], and the <<basic-auth>>.
Authentication will fail if either the <<basic-auth>> or the xref:security-oidc-bearer-token-authentication.adoc[OIDC bearer token authentication mechanism] failed to provide the `SecurityIdentity`.

In some situations, you may need to enable the <<inclusive-authentication>> only for certain HTTP paths with elevated security, like admin endpoints.
Here is example how to do it:

[source,properties]
----
quarkus.http.auth.inclusive=false

quarkus.http.auth.permission.admin-endpoints.paths=/admin
quarkus.http.auth.permission.admin-endpoints.policy=authenticated
quarkus.http.auth.permission.admin-endpoints.auth-mechanism=bearer,x509 <1>
quarkus.http.auth.permission.admin-endpoints.inclusive=true <2>
----
<1> The REST endpoint `/admin` can only be accessed by using both the xref:security-oidc-bearer-token-authentication.adoc[OIDC bearer token authentication mechanism], and the <<mutual-tls>>.
<2> Enable <<inclusive-authentication>> only for the REST endpoint `/admin`.
...
----
<1> The REST endpoint `/hello/basic` can only ever be accessed by using the <<basic-auth>>.
<2> The `/hello/basic` endpoint requires authentication, because when no standard security annotation accompanies the `@BasicAuthentication` annotation, the `@Authenticated` annotation is added by default.
...
<5> The REST endpoint `/hello/custom` can only ever be accessed by using a custom `HttpAuthenticationMechanism` with a `custom` authentication scheme.
<6> The `/hello/custom` endpoint requires authentication, because when no standard security annotation accompanies the `@HttpAuthenticationMechanism` annotation, the `@Authenticated` annotation is added by default.

[IMPORTANT]
====
Ensure that the value of the `@HttpAuthenticationMechanism` annotation matches the authentication scheme supported by `HttpAuthenticationMechanism`.
For example, for `@HttpAuthenticationMechanism("custom")` to work, a custom `io.quarkus.vertx.http.runtime.security.HttpAuthenticationMechanism` implementation must set its authentication scheme to `custom` in its `HttpAuthenticationMechanism#getCredentialTransport` method implementation.
====
