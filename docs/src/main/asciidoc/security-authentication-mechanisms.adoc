...
public class HelloResource {
    ...
    @GET
    @Path("basic-or-code-flow")
    public String basicOrCodeFlowAuthMechanism() {  <1>
        return "basic-or-code-flow";
    }
}
----
<1> The REST endpoint `/hello/basic-or-code-flow` can be accessed only if both the xref:security-oidc-code-flow-authentication.adoc[OIDC authorization code flow mechanism] and the <<basic-auth>> have successfully provided `SecurityIdentity`.
...
----
<1> The REST endpoint `/hello/basic-or-code-flow` uses both the xref:security-oidc-code-flow-authentication.adoc[OIDC authorization code flow mechanism] and the <<basic-auth>>.
Access is granted if at least one of them successfully provides `SecurityIdentity`.
