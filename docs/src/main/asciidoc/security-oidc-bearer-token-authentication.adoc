...
Use the `quarkus.oidc.dpop.proof-age` property to reject DPoP proofs older than the configured duration (default is 5 minutes), calculated from the proof's `iat` (issued at) claim.
This ensures proofs with timestamps in the past or future are properly rejected. The `quarkus.oidc.dpop.lifespan-grace` property can optionally add a grace period to tolerate slight clock skews.
The optional `exp` (expiry) claim is also verified when present.
