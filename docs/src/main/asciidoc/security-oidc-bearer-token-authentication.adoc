...
Use the `quarkus.oidc.dpop.proof-age` property to reject DPoP proofs older than the configured duration, calculated from the proof's required `iat` (issued at) claim. The optional `exp` (expiry) claim is verified when present.
