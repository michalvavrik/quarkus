...
public class ProtectedResource {
    ...
    @PermissionsAllowed("api\\:v1\\:read") <3>
    @GET
    @Path("/api")
    public String api() {
        return "api";
...
<3> Scopes with multiple colons such as `api:v1:read` require escaped colons (`\\:`) in the annotation value because only one permission-to-action separator is allowed and there is no way to determine which colon to use.
