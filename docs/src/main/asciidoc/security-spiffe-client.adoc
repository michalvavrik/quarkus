...
class MySpiffeClientTest {
    ...
    void testClientAuthentication() {
        // Test logic that relies on Keycloak SPIFFE client authentication
    }
}
----
...
To reuse an existing realm, export it to a JSON file and set the SPIFFE identity provider's `trustDomain` to `spiffe://test.quarkus.io` and `bundleEndpoint` to `http://host.testcontainers.internal:18443/bundle`.
The SPIFFE Dev Service serves the trust bundle on port `18443` and Keycloak reaches it from inside its container through `host.testcontainers.internal`.
