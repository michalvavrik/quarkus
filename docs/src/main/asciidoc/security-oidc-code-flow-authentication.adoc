...
The JWT-SVID audience defaults to the `quarkus.oidc.auth-server-url` value.
If the OIDC provider's issuer differs from `quarkus.oidc.auth-server-url`, set `quarkus.oidc.credentials.jwt.audience` explicitly.
...
NOTE: If you know that neither static nor dynamic OIDC tenants will require back-channel logout, you can disable its registration to avoid unnecessary overhead, see xref:security-oidc-expanded-configuration.adoc#allowed-routes-properties[Allowed Routes] for more information.
