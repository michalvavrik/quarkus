...
When multiple concurrent requests arrive with the same expired session, Quarkus deduplicates the refresh calls so that only one request refreshes the token and the others reuse the result.
The `quarkus.oidc.token.refresh-token-cache-time-to-live` property controls how long the result is cached (default is 5 seconds).
This is especially important for OIDC providers that issue single-use refresh tokens, where a duplicate refresh would be rejected.
