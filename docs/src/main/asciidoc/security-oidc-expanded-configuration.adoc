...
|quarkus.oidc.token.refresh-token-cache-time-to-live |5 seconds| Duration for which a completed refresh result is cached to deduplicate concurrent requests
...
`quarkus.oidc.token.refresh-token-cache-time-to-live` controls how long a completed token refresh result is kept in memory. When multiple concurrent requests carry the same expired session, only one refresh call is made and subsequent requests reuse the cached result. This is critical for OIDC providers that issue single-use refresh tokens, where a duplicate refresh is rejected. Set to `0S` to disable the cache.
