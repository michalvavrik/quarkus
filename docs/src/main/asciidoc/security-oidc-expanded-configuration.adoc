...
|quarkus.oidc.credentials.jwt.audience |Token audience|Audience (`aud`) claim value
...
When the xref:security-spiffe-client.adoc[SPIFFE Client] extension is present and no token path is set, the token is fetched directly from the local SPIRE Agent.
...
[NOTE]
====
If you know that neither static nor dynamic OIDC tenants will need to handle back-channel logout notifications, you can disable an HTTP route that accepts these notifications to avoid unnecessary overhead, see the <<allowed-routes>> section for more information.
====
...
https://datatracker.ietf.org/doc/html/rfc9449[RFC 9449] describes a Demonstrating Proof of Possession (DPoP) mechanism for cryptographically binding an access token to the client.
To enable the DPoP binding, set `quarkus.oidc.token.authorization-scheme=DPoP` to accept the DPoP access tokens sent as `Authorization: DPoP <token>`.
See the <<bearer-token-source-properties>> section for more information.

Use the following properties to configure the DPoP binding restrictions:
...
|quarkus.oidc.dpop.proof-age | 2 minutes | Maximum acceptable age of DPoP proof
|====

The `quarkus.oidc.dpop.proof-age` property can be used to configure the DPoP proof freshness validation.
Note that the `quarkus.oidc.token.lifespan-grace` property is applied as a grace period to tolerate slight clock skews when verifying the DPoP proof age and its optional expiry claim.
...
See the <<dpop-token-binding>> section for more information.
...
[NOTE]
====
If you know that neither static nor dynamic OIDC tenants will need to provide protected resource metadata, you can disable an HTTP route that supports protected resource metadata requests to avoid unnecessary overhead, see the <<allowed-routes>> section for more information.
====

[[allowed-routes]]
=== Allowed Routes

.OIDC allowed routes property
[options="header"]
|====
|Property name |Default |Description

|quarkus.oidc.allowed-routes |`backchannel-logout,resource-metadata`|OIDC routes that tenants can use
|====

To be able to support https://openid.net/specs/openid-connect-backchannel-1_0.html[OIDC back-channel logout] and https://datatracker.ietf.org/doc/rfc9728/[OAuth2 Protected Resource Metadata] requests for both static and dynamic OIDC tenants, `quarkus-oidc` must register routes that require dedicated HTTP handlers to manage such requests for all the OIDC tenants.

However, it is not always possible to figure out at start-up if such routes will actually be needed. For example, only dynamic OIDC tenants may choose to support one or all of the features these routes are meant to support.

This property can be used to inform the OIDC runtime that either some or all the supported routes are not needed, thus avoiding unnecessary overhead.

Supported values:

* `backchannel-logout` - enables an HTTP route for OIDC tenants to receive <<back-channel-logout,back-channel logout>> notifications
* `resource-metadata` - enables an HTTP route for OIDC tenants to handle requests for <<resource-metadata-properties,protected resource metadata>>

For example, if your tenants only need back-channel logout:

[source,properties]
----
quarkus.oidc.allowed-routes=backchannel-logout
----

If no tenant needs any of these routes, leave the property empty:

[source,properties]
----
quarkus.oidc.allowed-routes=
----
