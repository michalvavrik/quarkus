...
public final class AccessTokenRequestFilterGenerator {
    ...
    private String createUniqueClassName(RequestFilterKey i, AccessTokenInstanceBuildItem instance) {
        ...
        if (i.methodDescription != null) {
            String fullSignatureHash = instance.getTargetMethodInfo().declaringClass().name().toString() + "_"
                    + i.methodDescription.getMethodName() + "_" + Arrays.toString(i.methodDescription.getParameterTypes());
            // the full signature hash should limit collisions for overridden methods, we add the simple class
            // name to make the filter easier to identify
            uniqueClassName += "_" + instance.getTargetMethodInfo().declaringClass().simpleName() + "_"
                    + HashUtil.sha256(fullSignatureHash);
        }
