...
class EmailAuthenticationMechanism implements HttpAuthenticationMechanism {
    ...
    EmailAuthenticationMechanism(EmailAuthenticationConfig emailAuthenticationConfig, VertxHttpConfig vertxHttpConfig,
        ...
        final String key;
        if (vertxHttpConfig.encryptionKey().isEmpty()) {
            ...
            key = vertxHttpConfig.encryptionKey().get();
        }
        this.loginManager = new PersistentLoginManager(key, emailAuthenticationConfig.sessionCookie(),
                emailAuthenticationConfig.timeout().toMillis(),
                ...
                return findSecurityIdentityByPrincipalName(identityProviderManager, context, result.getPrincipal())
                        .invoke(securityIdentity -> {
                            if (securityIdentity == null || securityIdentity.isAnonymous()) {
                                LOG.debugf("Could not find valid SecurityIdentity for principal name '%s'",
                                        result.getPrincipal());
        ...
        return tokenStorage.findPrincipalNameByToken(token, routingContext)
                .flatMap(principalName -> {
                    ...
                    return findSecurityIdentityByPrincipalName(identityProviderManager, routingContext, principalName)
                            .onItem().ifNull()
                            .failWith(() -> new AuthenticationCompletionException(
                                    "Could not find SecurityIdentity with principal name '%s' (the principal name resolved for email authentication token '%s')"
                                            .formatted(principalName, token)));
                });
    }
                    ...
                    if (securityIdentity == null || securityIdentity.isAnonymous()) {
                        return Uni.createFrom().failure(new AuthenticationFailedException(
                                "Cannot find SecurityIdentity for principal name '" + principalName
                                        + "', the email authentication token won't be sent"));
                    }
