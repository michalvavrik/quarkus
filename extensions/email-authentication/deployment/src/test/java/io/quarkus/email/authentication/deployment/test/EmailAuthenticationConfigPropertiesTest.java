...
class EmailAuthenticationConfigPropertiesTest {
    ...
    private static final TestEmailAuthenticationHelper HELPER = new TestEmailAuthenticationHelper(
            "/secured/gen-token", "/secured/post-location", "code is here!", "token is ",
    ...
    static final QuarkusUnitTest APP = new QuarkusUnitTest()
            ...
            .withRuntimeConfiguration("""
                    ...
                    quarkus.email-authentication.token-generation-location=secured/gen-token
                    quarkus.email-authentication.token-length=7
                    quarkus.email-authentication.token-parameter=token-param
                    quarkus.email-authentication.token-page=secured/token-form.html
    ...
    void testCompleteFlowSuccess() {
        ...
        HELPER.requestTokenFor(username).statusCode(302).header("location", containsString("/secured/token-form.html"));
        cookie = HELPER.assertCookiePresent("token-req");
    ...
    void testValidTokenWithoutRequestCookie() {
        ...
        HELPER.requestTokenFor(username).statusCode(302).header("location", containsString("/secured/token-form.html"));
        HELPER.assertCookiePresent("token-req");
        ...
        HELPER.submitToken(token).statusCode(302).header("location", containsString("/err.html"));
    }

    @Test
    void testTokenExpirationTime() throws InterruptedException {
        String email = "albus@quarkus.io";
        TestTrustedIdentityProvider.addUser(email);

        HELPER.requestTokenFor(email).statusCode(302).header("location", containsString("/secured/token-form.html"));
        HELPER.assertCookiePresent("token-req");
        String token = HELPER.assertEmailAndGetToken(mailbox, email);
        assertThat(token).hasSize(7);

        // token should expire in 3 seconds
        Thread.sleep(3300);
