...
final class TestEmailAuthenticationHelper {
    ...
    TestEmailAuthenticationHelper(String tokenGenerationPath, String postLocation, String emailSubject, String emailTextStart,
            String usernameParameter, String tokenParameter) {
    ...
    ValidatableResponse requestTokenFor(String username) {
        ...
        return RestAssured.given()
                .filter(cookieFilter)
                .formParam(usernameParameter, username)
    ...
    void clear() {
        ...
        TestTrustedIdentityProvider.reset();
    }
    ...
    String assertEmailAndGetToken(MockMailbox mailbox, String emailAddress) {
        ...
        String emailAuthToken = extractToken(emails.get(0).getText());
        assertThat(emailAuthToken).isNotEmpty();
        return emailAuthToken;
    }

    static String extractToken(String emailText) {
        return emailText
                .trim()
                .transform(t -> {
                    var arr = t.split(" ");
    ...
    ValidatableResponse submitToken(String token) {
        RestAssured.enableLoggingOfRequestAndResponseIfValidationFails();
        return RestAssured.given()
                .filter(cookieFilter)
                .redirects().follow(false)
                .formParam(tokenParameter, token)
                .post(postLocation)
                .then();
    }
