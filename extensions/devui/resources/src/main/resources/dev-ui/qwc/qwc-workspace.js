...
export class QwcWorkspace extends observeState(QwcHotReloadElement) { 
    ...
    async afterRender() {
        ...
        if(this._workspaceItems && this._workspaceItems.size>0){
            ...
            if(this._selectedWorkspaceItem.name){
                const filePath = this._toUnixPath(this._selectedWorkspaceItem.name);
                ...
                const filePath = this._toUnixPath(selectedItem.name);
                directoryTree.selectFile(filePath);
    ...
    _onFileSelect(event) {
        const filePath = this._correctPathSeparators(event.detail.file);
    ...
    _toUnixPath(filePath) {
        ...
        if (this._fileSeparator !== '/') {
            return filePath.split(this._fileSeparator).join('/');
        }
        return filePath;
    }

    _correctPathSeparators(filePath) {
        // this deals with a situation where 'qui-directory-tree' can produce path with unix separators on windows
        // TODO: drop this once the 'qui-directory-tree' component uses OS-specific file path separators
        if (this._fileSeparator !== '/') {
            return filePath.split('/').join(this._fileSeparator);
        }
        return filePath;
    }
}
