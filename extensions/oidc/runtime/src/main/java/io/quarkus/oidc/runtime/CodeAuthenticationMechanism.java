        ...
        return removeSessionCookie(context, sessionCookieConfig)
                .chain(new Function<Void, Uni<? extends ChallengeData>>() {
                    ...
                    public Uni<ChallengeData> apply(Void t) {
                        ...
                        boolean pushedAuthorizationRequest = authenticationConfig.par().enabled().orElse(false);

                        if (!pushedAuthorizationRequest) {
                            // for par, we add the client_id with the client credentials
                            // client_id
                        ...
                        if (pushedAuthorizationRequest) {
                            return configContext.getOidcProviderClient()
                                    .doPushedAuthorizationRequest(codeFlowParams.toString())
                                    .map(requestUri -> {
                                        // client_id
