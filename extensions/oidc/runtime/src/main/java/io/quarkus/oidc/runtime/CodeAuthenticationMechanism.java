        ...
        return removeSessionCookie(context, sessionCookieConfig)
                .chain(new Function<Void, Uni<? extends ChallengeData>>() {
                    ...
                    public Uni<ChallengeData> apply(Void t) {
                        ...
                        boolean rarEnabled = !authenticationConfig.rar().stringType().isEmpty(); // 'type' is required
                        if (rarEnabled) {
                            var authorizationDetailsArray = new JsonArray();
                            var authorizationDetailsObject = new JsonObject();
                            authenticationConfig.rar().stringType().forEach(authorizationDetailsObject::put);
