...
public class OidcClientFilterDeploymentHelper<T extends AbstractTokensProducer> {
    ...
    public String getOrCreateNamedTokensProducerFor(String oidcClientName, AnnotationInstance annotationInstance) {
        ...
        if (targetMethod != null && clientNameToGeneratedClass.containsKey(new TokensProducerKey(oidcClientName, null))) {
            throwBothMethodAndClassAnnotated(annotationInstance);
        } else {
            boolean appliedOnMethod = clientNameToGeneratedClass.keySet().stream()
                    .filter(k -> k.methodInfo != null)
                    .map(TokensProducerKey::clientName)
                    .anyMatch(oidcClientName::equals);
            if (appliedOnMethod) {
                throwBothMethodAndClassAnnotated(annotationInstance);
            }
        }
