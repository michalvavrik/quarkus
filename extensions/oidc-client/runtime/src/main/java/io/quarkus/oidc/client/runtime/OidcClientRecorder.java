...
public class OidcClientRecorder {
    ...
    protected static Uni<OidcClient> createOidcClientUni(OidcClientConfig oidcConfig, String oidcClientId,
        ...
        if (tokenPathIsAbsoluteUrl || !oidcConfig.discoveryEnabled().orElse(true)) {
            ...
            // create OIDC client form discovered OIDC metadata
            final Uni<OidcClient> deferredOidcClient = Uni.createFrom()
                    .deferred(() -> discoverTokenUris(client, oidcRequestFilters, oidcResponseFilters,
                            ...
                            .flatMap(oidcConfigurationMetadata -> createOidcClientUniFromMetadata(oidcConfigurationMetadata,
                                    oidcConfig, client, oidcRequestFilters, oidcResponseFilters, vertx)));
            return deferredOidcClient.onFailure().recoverWithItem(throwable -> {
                Uni<OidcClient> nonRecoverableClient = deferredOidcClient.onFailure()
                        .transform(t -> toOidcClientException(getEndpointUrl(oidcConfig), t));
                return new LazyOidcClient(nonRecoverableClient);
            });
        }
    }
