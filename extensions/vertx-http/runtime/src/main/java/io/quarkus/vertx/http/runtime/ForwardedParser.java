        ...
        if (selector != null) {
            ...
            if (!forValues.isEmpty()) {
                ...
                try {
                    ...
                    log.debugf(e, "Rejecting request: the X-Forwarded-For selector threw an exception");
                    rejected = true;
                    return null;
                }
                if (selected == null) {
                    log.debug("Rejecting request: the X-Forwarded-For selector did not return a value");
                    rejected = true;
                    return null;
                }
                setForwardedFor(selected, xForwardedValues);
            }
        } else {
