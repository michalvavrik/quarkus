...
public interface FormAuthenticationToken {
    ...
    /**
     * The cookie path for the session, location, and token request cookies.
     */
    @WithDefault("/")
    Optional<String> cookiePath();

    /**
     * Cookie domain parameter value which, if set, will be used for the session, location and request cookies.
     */
    ...
    /**
     * Location, where the form authentication token submitted on the login page can be posted.
     */
    @WithDefault("/j_security_check")
    String postLocation();

    /**
     * Token request location. Typically, users submit email to a form, which is posted to this location.
     */
    @WithDefault("/j_security_token")
    String tokenRequestPath();
