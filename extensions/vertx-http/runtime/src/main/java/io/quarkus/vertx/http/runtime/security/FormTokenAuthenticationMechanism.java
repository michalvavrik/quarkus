        ...
        if (isPostLocation(context)) {
            ...
            if (result != null) {
                ...
                return identityProviderManager
                        .authenticate(setRoutingContextAttribute(new TrustedAuthenticationRequest(result.getPrincipal()), context))
                        .onItem().ifNotNull().transform(new Function<SecurityIdentity, SecurityIdentity>() {
                            @Override
                            public SecurityIdentity apply(SecurityIdentity identity) {
                ...
                exchange.request().endHandler(new Handler<Void>() {
                    ...
                    public void handle(Void event) {
                        try {
                            ...
                            securityContext
                                    .authenticate(setRoutingContextAttribute(new TrustedAuthenticationRequest(principal), exchange))
