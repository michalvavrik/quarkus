...
public final class FormTokenAuthenticationMechanism implements HttpAuthenticationMechanism {
    ...
    public Uni<SecurityIdentity> authenticate(RoutingContext context, IdentityProviderManager identityProviderManager) {
        if (isPostLocation(context)) {
                ...
                public void accept(UniEmitter<? super SecurityIdentity> uniEmitter) {
                    context.request().endHandler(new Handler<Void>() {
                        ...
                        public void handle(Void event) {
                            try {
                                final String jUsername = context.request().getFormAttribute(usernameParameter);
                                if (jUsername == null || jUsername.isBlank()) {
                                    LOG.debugf("Could not generate token as username form param '%s' was not present "
                                                    + "in the posted result for %s", usernameParameter, context);
                                    uniEmitter.complete(null);
                                    return;
                                } else if (LOG.isTraceEnabled()) {
                                    LOG.tracef("Received request to generate token for user with username '%s'", jUsername);
                                }
                                identityProviderManager
                                        .authenticate(
                                                setRoutingContextAttribute(new TrustedAuthenticationRequest(jUsername), context))
                                        .subscribe().with(new Consumer<SecurityIdentity>() {
                                            ...
                                            public void accept(Throwable throwable) {
                                                if (LOG.isDebugEnabled()) {
                                                    LOG.debug("Denying token request - unable to verify '" + jUsername + "' username", throwable);
                                                }
            ...
            // FIXME: EITHER GENERATE THE TOKEN,
            //  send and redirect, or if invalid (or too many),
            //  respond with 401? hard to say
                        ...
                        try {
                            ...
                            securityContext
                                    ...
                                    .subscribe().with(new Consumer<SecurityIdentity>() {
                                        ...
                                        @Override
                                        public void accept(Throwable throwable) {
                                            LOG.debug("Unable to complete post authentication", throwable);
                                            uniEmitter.fail(throwable);
                                        }
                                    });
                        } catch (Throwable t) {
