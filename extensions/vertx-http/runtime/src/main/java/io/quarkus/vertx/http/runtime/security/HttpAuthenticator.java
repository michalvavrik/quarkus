...
public final class HttpAuthenticator {
    ...
    /**
     * This is {@link RoutingContext} data key added for an array of authentication mechanisms which produced
     * {@link SecurityIdentity}.
     */
    private static final String MECHANISMS_WHICH_PRODUCED_IDENTITIES = HttpAuthenticator.class.getName()
            + "#mechanisms-which-produced-identities";
                    ...
                    .transformToUni(new Function<HttpAuthenticationMechanism[], Uni<? extends SecurityIdentity>>() {
                        ...
                        public Uni<SecurityIdentity> apply(HttpAuthenticationMechanism[] candidates) {
                            return createAndValidateSecurityIdentity(routingContext, candidates, identityProviderManager,
                                    inclusiveAuth, strictInclusiveMode, pathSpecificMechanism.names().size());
                        }
                    });
        }
                ...
                .onItem().transformToUni(new Function<SecurityIdentity, Uni<? extends SecurityIdentity>>() {
                    ...
                    public Uni<SecurityIdentity> apply(SecurityIdentity identity) {
                        if (identity != null) {
                            ...
                            if (selectAuthMechanismWithAnnotation && !isAuthMechanismSelected(routingContext)) {
                                return rememberAuthMechScheme(mechanisms[i], routingContext, inclusiveAuth)
    ...
    private Uni<HttpAuthenticationMechanism[]> findBestCandidateMechanisms(RoutingContext routingContext, boolean inclusiveAuth,
            Set<String> pathSpecificMechanisms) {
        ...
        return findBestCandidateMechanisms(routingContext, 0, inclusiveAuth, pathSpecificMechanisms, new LinkedList<>())
                .map(list -> {
                    if (list == null || list.isEmpty()) {
                        return null;
                    }
                    return list.toArray(HttpAuthenticationMechanism[]::new);
                });
    }

    private Uni<List<HttpAuthenticationMechanism>> findBestCandidateMechanisms(RoutingContext routingContext, int i,
            boolean inclusiveAuth, Set<String> pathSpecificMechanisms, List<HttpAuthenticationMechanism> selectedMechanisms) {
        if (i == mechanisms.length) {
        ...
        return getPathSpecificMechanism(i, routingContext, pathSpecificMechanisms, inclusiveAuth).onItem().transformToUni(
                new Function<HttpAuthenticationMechanism, Uni<? extends List<HttpAuthenticationMechanism>>>() {
                    ...
                    public Uni<? extends List<HttpAuthenticationMechanism>> apply(HttpAuthenticationMechanism mech) {
                        if (mech != null) {
                            if (selectAuthMechanismWithAnnotation && !isAuthMechanismSelected(routingContext)) {
                                if (pathSpecificMechanisms.isEmpty()) {
                                    // we found all required authentication mechanisms
                                    return rememberAuthMechScheme(mech, routingContext, inclusiveAuth)
                                            .replaceWith(selectedMechanisms);
                                }
                                // remember this one, and find another required one
                                return rememberAuthMechScheme(mech, routingContext, inclusiveAuth)
                                        .chain(() -> findBestCandidateMechanisms(routingContext, i + 1, inclusiveAuth,
                                                pathSpecificMechanisms, selectedMechanisms));
                            }
                            if (pathSpecificMechanisms.isEmpty()) {
                                // we found all required authentication mechanisms
                                return Uni.createFrom().item(selectedMechanisms);
                        ...
                        // find another required mechanism
                        return findBestCandidateMechanisms(routingContext, i + 1, inclusiveAuth, pathSpecificMechanisms,
                                selectedMechanisms);
                    }
                });
    }
                ...
                .transform(new Function<HttpCredentialTransport, HttpAuthenticationMechanism>() {
                    ...
                    public HttpAuthenticationMechanism apply(HttpCredentialTransport t) {
                        if (t != null && pathSpecificMechanisms.contains(t.getAuthenticationScheme())) {
                            ...
                            pathSpecificMechanisms.remove(t.getAuthenticationScheme());
    ...
    private static Collection<HttpAuthenticationMechanism> getMechanismsWhichProducedIdentity(RoutingContext routingContext) {
        return routingContext.get(MECHANISMS_WHICH_PRODUCED_IDENTITIES);
    }

    private static boolean hasMechanismsWhichProducedIdentity(RoutingContext routingContext) {
        return getMechanismsWhichProducedIdentity(routingContext) != null;
    }
