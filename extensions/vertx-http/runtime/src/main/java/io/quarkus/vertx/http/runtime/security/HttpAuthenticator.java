...
public final class HttpAuthenticator {
    ...
    static void selectAuthMechanism(RoutingContext routingContext, String[] authMechanisms) {
        if (authenticationAlreadyAttempted(routingContext, authMechanisms)) {
            ...
            throw new AuthenticationFailedException("""
                    ...
                    It can happen if the '%1$s' are selected with an annotation but '%2$s' are activated by
                    ...
                    which secure the same REST endpoints as the ones secured by the '%1$s' authentication mechanisms
                    selected with the annotation.
                    """.formatted(Arrays.toString(authMechanisms), previousMechanisms));
        }
        // this must be PUT, not the mechanism aggregation; at this point, we know that authentication did not happen,
        // or that it happened using the identical mechanisms that were selected with the annotation
        // (because we would fail otherwise, see the IF above) and we document that annotation-based authentication
        // has priority the path-based authentication selected with HttpSecurityPolicy
        routingContext.put(SELECTED_AUTH_MECHANISMS, new AuthenticationMechanisms(authMechanisms));
    }
    ...
    private static boolean authenticationAlreadyAttempted(RoutingContext event, String[] newAuthMechanisms) {
        return event.get(ATTEMPT_AUTH_INVOKED) == TRUE
                && authenticationAttemptedWithDifferentRequirements(newAuthMechanisms, event);
    }

    private static boolean authenticationAttemptedWithDifferentRequirements(String[] newAuthMechanisms, RoutingContext event) {
        // this is configured used for the previous attempt, we require that this is identical to what is selected
        // by the annotation
        AuthenticationMechanisms previouslySelectedMechanisms = getSelectedAuthMechanisms(event);

        if (previouslySelectedMechanisms != null) {
            ...
            Set<String> previouslySelectedMechanismNames = previouslySelectedMechanisms.names();
            if (previouslySelectedMechanismNames.size() == newAuthMechanisms.length) {
                for (String newAuthMechanism : newAuthMechanisms) {
                    if (!previouslySelectedMechanismNames.contains(normalizeMechanismName(newAuthMechanism))) {
                        return true;
                    }
                }
                return false;
