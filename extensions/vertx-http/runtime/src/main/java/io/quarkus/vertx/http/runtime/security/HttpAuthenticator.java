...
public final class HttpAuthenticator {
    ...
    private static void rememberSelectedAuthMechanism(RoutingContext event, String newAuthMechanism, boolean inclusiveAuth) {
        ...
        if (currentMechanisms == null) {
            event.put(SELECTED_AUTH_MECHANISMS, new AuthenticationMechanisms(Set.of(newAuthMechanism), inclusiveAuth));
        } else {
            Set<String> newMechanisms = new HashSet<>(currentMechanisms.names());
            newMechanisms.add(newAuthMechanism);
    ...
    private static HttpAuthenticationMechanism[] getSelectedAuthMechanismInstances(RoutingContext event) {
        List<HttpAuthenticationMechanism> selectedInstances = event.get(SELECTED_AUTH_MECHANISM_INSTANCES);
    ...
    private static HttpAuthenticationMechanism[] getMechanismsWhichProvidedIdentity(RoutingContext routingContext) {
        List<HttpAuthenticationMechanism> mechanisms = routingContext.get(MECHANISMS_WHICH_PROVIDED_IDENTITY);
        if (mechanisms == null || mechanisms.isEmpty()) {
            return null;
        }
        return mechanisms.toArray(HttpAuthenticationMechanism[]::new);
    }
