...
public interface TrustedProxyCheck {
    ...
    static TrustedProxyCheck createTruststoreCheck(HttpServerRequest event, Collection<X509TrustManager> trustManagers) {
        ...
        for (X509TrustManager tm : trustManagers) {
            try {
                ...
                LOGGER.debug("Client certificate chain matches trusted proxy alias");
                return allowAll();
            } catch (CertificateException e) {
                LOGGER.tracef(e, "Client certificate chain did not match trusted proxy alias");
            }
        }

        LOGGER.debug("Client certificate chain does not match any trusted proxy alias");
        return denyAll();
    }
