...
@TestHTTPEndpoint(SecuredRepositoriesResource.class)
@QuarkusTest
public class HibernateOrmDataSecurityTest {
    ...
    void testMethodLevelRolesAllowed() {
        testMethodLevelPermitAllByRetrievingEntity("data-sub", "foo").statusCode(404);
        ...
        testMethodLevelPermitAllByRetrievingEntity("data-sub", "foo").statusCode(200);
        testMethodLevelPermitAllByDeletingEntity("data-sub", "foo");
    }
    ...
    private void testMethodLevelPermitAllByDeletingEntity(String subResourceName, String entityName) {
        given()
                .pathParam("name", entityName)
                .contentType(ContentType.JSON)
                .pathParam("sub-resource-name", subResourceName)
                .when().delete("/{sub-resource-name}/by/name/{name}")
                .then()
                .statusCode(204);
        testMethodLevelPermitAllByRetrievingEntity(subResourceName, entityName).statusCode(404);
    }

    private ValidatableResponse testMethodLevelPermitAllByRetrievingEntity(String subResourceName, String entityName) {
        return given()
                .pathParam("name", entityName)
                .contentType(ContentType.JSON)
                .pathParam("sub-resource-name", subResourceName)
                .when().get("/{sub-resource-name}/by/name/{name}")
                .then();
    }
}
